...
# 2. Create Internet Gateway Resource
resource "aws_internet_gateway" "gw" {
  vpc_id = aws_vpc.test.id
}

# 3. Create Route Table Resource
...
# 6. Create Security Group to allow port 22,80,443,4000,4001,4002
...
resource "aws_instance" "web-server-instance" {
  ...
  provisioner "remote-exec" {
    ...
    connection {
    type     = "ssh"
    private_key = file("./test.pem") # privte key
    user     = "ubuntu"
